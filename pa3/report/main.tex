...
% WE ALSO PERFORM FILTERING ON THE CHUNKS, DESCRIBE HOW

\subsection{Retrieval and reranking}
For evidence-grounded answering, the pipeline embeds the user query, retrieves the top-$k$ most similar text segments from pgvector
using a configurable similarity metric (cosine, L2, or L1), and reranks the candidates with a cross-encoder.
The final top-$n$ segments are formatted into a structured context block that is provided to the LLM as explicit evidence.
